import React, { Component } from "react";
export class Classformvalid extends Component{
    constructor(){
        super()
        this.state={fill:false,prtname:'laptop 1',prtprice:'12000',prtimg:'data:image/jpeg;base64,',arr:[],inx:'',isedit:false}
    }
    render(){
        let find=(e)=>{
            if(e.target.name==='pname'){
                this.setState({prtname:e.target.value})
                console.log(e.target.value)
            }
            else if(e.target.name==='pprice'){
                this.setState({prtprice:e.target.value})
                console.log(e.target.value);
            }
            else{
                this.setState({prtimg:e.target.value})
                console.log(e.target.value);
            }
        }
        let click=(e)=>{
            e.preventDefault()
            if(this.state.prtname===''|| this.state.prtprice==='' || this.state.prtimg===''){
                this.setState({fill:true})
            }
            else if(this.state.isedit){
                let obj={name:this.state.prtname,price:this.state.prtprice,img:this.state.prtimg}
                let find=this.state.arr.map((e,i)=>{return i===this.state.inx? obj:e})
                this.setState({arr:find})
                this.setState({prtname:""})
            this.setState({prtprice:""})
            this.setState({prtimg:""})
                this.setState({isedit:false})
            }
            else{
                let obj={name:this.state.prtname,price:this.state.prtprice,img:this.state.prtimg}
            this.setState({arr:[...this.state.arr,obj]})
            this.setState({prtname:""})
            this.setState({prtprice:""})
            this.setState({prtimg:""})
            this.setState({fill:false})
            }
            
            // console.log(obj);

        }
        let del=(index)=>{
            let x=this.state.arr.filter((e,i)=>{return i!==index ? e :'' })
            this.setState({arr:x})
        }
        let edit=(v,i)=>{
            this.setState({prtname:v.name})
            this.setState({prtprice:v.price})
            this.setState({prtimg:v.img})
            this.setState({inx:i})
            this.setState({isedit:true})
            console.log(v)
        }
        return(
            <div>
                <form>
                    <div>
                        <label>Product Name</label>
                        <input type="text" name="pname" value={this.state.prtname} onChange={find}/>
                        {this.state.fill && this.state.prtname===''? <p>Please fill out the field</p>:''}
                    </div>
                    <div>
                        <label>Product Price</label>
                        <input type="number" name="pprice" value={this.state.prtprice} onChange={find}/>
                        {this.state.fill&&this.state.prtprice===''? <p>Please fill out the field</p>:''}
                    </div>
                    <div>
                        <label>Product Image</label>
                        <input type="text" value={this.state.prtimg} onChange={find}/>
                        {this.state.fill && this.state.prtimg===''? <p>Please fill out the field</p>:''}
                    </div>
                    <button onClick={click}>submit</button>
                </form>
                <div style={{display:"flex",flexWrap:"wrap",width:"85%",margin:"auto"}}>
                    {
                        this.state.arr.map((e,i)=>{
                            return (
                                <div key={i} style={{width:"20%",padding:"10px",boxShadow:"0 0 10px #000"}}>
                                    <img src={e.img} alt='pic' style={{width:"100%"}}/>
                                    <h3>{e.name}</h3>
                                    <h2>{e.price}</h2>
                                    <button onClick={()=>del(i)}>Delete</button>
                                    <button onClick={()=>edit(e,i)}>Edit</button>
                                </div>
                            )
                        })
                    }
                </div>
            </div>
        )
    }
}